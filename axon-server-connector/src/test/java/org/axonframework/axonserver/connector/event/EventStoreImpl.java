...
public class EventStoreImpl extends EventStoreGrpc.EventStoreImplBase {
    ...
    private final List<Event> snapshots = new LinkedList<>();
    ...
    public void listAggregateEvents(GetAggregateEventsRequest request, StreamObserver<Event> responseObserver) {
        Optional<Event> snapshot = snapshots.stream()
                                            .filter(e -> Objects.equals(e.getAggregateIdentifier(), request.getAggregateId()))
                                            .max(Comparator.comparing(Event::getAggregateSequenceNumber));
        snapshot.ifPresent(responseObserver::onNext);

        events.stream().filter(e -> e.getAggregateIdentifier().equals(request.getAggregateId()))
              .filter(e -> e.getAggregateSequenceNumber() > snapshot.map(Event::getAggregateSequenceNumber).orElse(-1L))
